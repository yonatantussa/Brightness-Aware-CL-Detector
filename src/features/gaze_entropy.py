...
def _entropy(points, grid_size, path_world_video):
    ...
    gaze_x = gaze[:, 0]
    gaze_y = gaze[:, 1]
    x_counter = np.floor(gaze_x / (width / grid_size))
    y_counter = np.floor(gaze_y / (height / grid_size))
    hist = np.histogram2d(x_counter, y_counter, bins=grid_size)[0]

    hist_flatten = hist.flatten()
    ent = entropy(hist_flatten)
    return ent

def gaze_entropy(data_path, processed_step_path, grid_size=15, seconds=1):
    ...
    gaze_ent = Parallel(n_jobs=cpus)(delayed(_entropy)(gaze[i:min(i + seconds * 30, len(gaze))], grid_size, world_video) for i in tqdm(range(0, len(gaze))))
